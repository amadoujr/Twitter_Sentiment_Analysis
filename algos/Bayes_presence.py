...
class Bayes_presence:
    ...
    def delete_copy(self, res_categorie):
        for idx, row in enumerate(res_categorie):
            words = row[1].split() 
            seen_words = set() 
            new_words = []
            for word in words:
                if word not in seen_words:
                    ...
                    seen_words.add(word)
            res_categorie[idx][1] = ' '.join(new_words)
        return res_categorie
    
    def bayes_presence(self, dataFrameTrain, dataFrameTest):
        ...
        res_negative_without_copy = self.delete_copy(res_negative)
        res_neutre_without_copy = self.delete_copy(res_neutre)
        res_positive_without_copy = self.delete_copy(res_positive)
        
        text_negative = ' '.join([row[1] for row in res_negative_without_copy])
        list_words_negative = text_negative.split()
        text_neutre = ' '.join([row[1] for row in res_neutre_without_copy])
        list_words_neutre = text_neutre.split()
        text_positive = ' '.join([row[1] for row in res_positive_without_copy])
